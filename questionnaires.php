...
require_once "connection.php";
        ...
        $requete = $conn->prepare("SELECT * FROM qcm WHERE niveau = :level ORDER BY RAND() LIMIT 10");

    // Sélection aléatoire de 10 questions pour le niveau de l'utilisateur
    $requete->execute(array(":level" => $_SESSION["niveau"]));
    $etudiant = $requete->fetchAll(PDO::FETCH_ASSOC);

// Initialisation des variables de session
    $_SESSION["score"] = 0;
    $_SESSION["tb"] = sizeof($etudiant);
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $score = 0;
    foreach ($etudiant as $row) {
        $reponseKey = "reponse" . $row["numQuestion"];
        if (isset($_POST[$reponseKey])) {
            $reponseUtilisateur = strip_tags($_POST[$reponseKey]);
            $idReponse = $row["reponse_correcte"];
            if ($reponseUtilisateur == $row["reponse1"]) {
                $score++;
            } else {
                $_SESSION["true"] = "";
                //$_SESSION["true"] = $row["reponse".$idReponse];
                $_SESSION["false"] = $reponseUtilisateur;
            }
        }
    }
...
<main id="container" style="margin: 200px auto 0 auto;width: 600px;font-size: 20px">
            ...
            <form method="POST" id="formulaire">
                <div class="form-group" id="form1">
                    ...
                    <div class="header col-12">
                        <h3 style="font-weight: bolder; "  id="intro1">QUESTIONNAIRE</h3><br>
                        <h3 style="font-weight: bolder; ">BIENVENUE, <?= $_SESSION["nomUtilisateur"] ?></h3><br>
                        <h6 id="intro2">Choisissez la bonne réponse</h6><br>
                        <div style="text-align: left">
                            <?php
                            if (!empty($etudiant)) {
                                foreach($etudiant as $row) {
                                    echo "<h3>" . $row['question'] . "</h3>";
                                    echo "<input required type='radio' name='reponse" . $row['numQuestion'] . "' value='" . $row['reponse1'] . "'>    " . $row['reponse1'] . "<br>";
                                    echo "<input required type='radio' name='reponse" . $row['numQuestion'] . "' value='" . $row['reponse2'] . "'>    " . $row['reponse2'] . "<br>";
                                    echo "<input required type='radio' name='reponse" . $row['numQuestion'] . "' value='" . $row['reponse3'] . "'>    " . $row['reponse3'] . "<br>";
                                    echo "<input required type='radio' name='reponse" . $row['numQuestion'] . "' value='" . $row['reponse4'] .  "'>    ". $row['reponse4'] . "<br><br>";
                                }
                            } else {
